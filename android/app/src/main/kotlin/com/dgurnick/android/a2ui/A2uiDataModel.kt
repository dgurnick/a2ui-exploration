...
/**
 * Stores the A2UI data model for a single surface as a mutable JSON-like tree.
 * Supports the [DataEntry] adjacency list format from [DataModelUpdateMessage].
 */
class A2uiDataModel {
    ...
    fun applyUpdate(payload: DataModelUpdatePayload) {
        val targetMap = if (payload.path.isNullOrBlank() || payload.path == "/") {
            root
        } else {
    ...
    fun resolveBoolean(path: String): Boolean? = when (val v = resolve(path)) {
        is Boolean -> v
    ...
    fun write(path: String, value: Any?) {
        ...
        val map = if (parentPath.isEmpty() || parentPath == "/") root
    ...
    private fun resolveDataEntry(entry: DataEntry): Any? = when {
    ...
    private fun navigateOrCreate(from: MutableMap<String, Any?>, path: String): MutableMap<String, Any?> {
        val parts = path.removePrefix("/").split("/").filter { it.isNotEmpty() }
        var current = from
        for (part in parts) {
            @Suppress("UNCHECKED_CAST")
...
/**
 * Resolves a [BoundValue] against the data model.
 * If both [BoundValue.path] and a literal are present, writes the literal to the
 * data model (initialization shorthand) and returns the literal.
 */
fun BoundValue.resolve(model: A2uiDataModel): Any? {
    return when {
        path != null && literalString != null -> { model.write(path, literalString); literalString }
        path != null && literalNumber != null -> { model.write(path, literalNumber); literalNumber }
        path != null && literalBoolean != null -> { model.write(path, literalBoolean); literalBoolean }
